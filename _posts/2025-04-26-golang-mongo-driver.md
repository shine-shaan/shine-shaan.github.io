...
java-script 데이터 전달을 위해 만들어진 JSON을 binary 형태의 데이터가 입력되도록 변형된 타입이다.
...
MongoDB 고 (Go) 드라이버 - 고 (Go) 드라이버 v2.2 - MongoDB Docs
https://www.mongodb.com/ko-kr/docs/drivers/go/current/ 
공식 문서를 이용해 Mongodb와 Go 프로젝트를 연결해 보겠다.
...
Mongo client는 database 및 collection 접근을 위한 필수 연결점이다.
...
위에서 얻은 collection을 이용해 CRUD 하는 방법은 아래와 같다.
